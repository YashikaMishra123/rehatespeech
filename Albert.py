...
# Create an ALBERT tokenizer
albert_tokenizer = AlbertTokenizer.from_pretrained('albert-base-v2')

# Create an ALBERT model
...
def create_model():
    ...
    albert_layer = albert_model(inps, attention_mask=masks)[0][:,0,:]
    dense = Dense(512,activation='relu',kernel_regularizer=regularizers.l2(0.02))(albert_layer)
...
def create_model():
    ...
    masks= Input(shape = (max_len,), dtype='int64')
    albert_layer = albert_model(inps, attention_mask=masks)[0][:,0,:]
    dense = Dense(512,activation='sigmoid',kernel_regularizer=regularizers.l2(0.01))(albert_layer)
    dropout= Dropout(0.5)(dense)
...
def create_model():
    inps = Input(shape = (max_len,), dtype='int64')
    masks= Input(shape = (max_len,), dtype='int64')
    albert_layer = albert_model(inps, attention_mask=masks)[0]
     # Apply a global average pooling layer to summarize token embeddings
    avg_pooling = tf.keras.layers.GlobalAveragePooling1D()(albert_layer)
     # Apply a global average pooling layer to summarize token embeddings
...
for sent in sentences:
    albert_inps=albert_tokenizer.encode_plus(sent,add_special_tokens = True,max_length =max_len,pad_to_max_length = True,return_attention_mask = True,truncation=True)
    input_ids.append(albert_inps['input_ids'])
    attention_masks.append(albert_inps['attention_mask'])
...
pickle_inp_path='Albert_inp.pkl'
pickle_mask_path='Albert_mask.pkl'
pickle_label_path='.albert_label.pkl'
...
log_dir='albert_model'
model_save_path='./albert_model.h5'
...
albert_model.compile(loss=loss, optimizer=optimizer, metrics=[metric])
...
history=albert_model.fit([train_inp,train_mask],train_label,batch_size=128,epochs=30,validation_data=([val_inp,val_mask],val_label),callbacks=callbacks)
